...
## 深入clip的渲染
clip仅仅决定那部分是可见的，非可见部分无法响应点击事件
虽然视觉上隐藏，但是元素的尺寸依然是原本的尺寸（clientWidth clientHeight并没有发生变化），存在一个兼容性问题在IE/Firefox下浏览器抹掉了不可见区域尺寸对布局的影响。
...
## relative与定位
相对自身，无侵入
margin移动元素和相对定位移动元素的区别？
https://demo.cssworld.cn/6/6-1.php（点击图片）：relative定位不会影响周围元素布局
相对定位的left/right/top/bottom的百分比值是相对于包含块计算的
top/bottom的百分比计算值和height的百分比计算值是一样的，都是相对高度计算的，如果包含块的高度是auto，那么计算值是0，偏移无效。
## relative最小化影响原则
尽量不使用relative, 能否使用"无依赖的绝对定位"
relative最小化
