...
1. 显示判断 props 以及 context 是否发生了变化 【TODO】hasContextChanged，如何得知context的变化的
...
2. 如果props没有更新 && context也没有发生变化 && workInProgress.expirationTime < nextRenderExpirationTime
    - 说明当前节点不需要更新，这种情况会有哪些操作呢？
        - didReceiveUpdate = false // 作用？updateFunctionComponent updateForwardRef 代码中搜索下，目前这两个类型节点的更新会用到这个标识 【TODO】
...
>【划重点】：WrapperCompo 发生了this.setState，在该组件的updateClassInstanxe过程中hi重新调用instance.render，此时会调用React.createElement重新生成元素，因此
...
获取fiber依赖的context链表【TODO】在哪设置的呢？是否存在这个链表<br/>
- 存在的话，存在循环遍历context依赖链表，看看是否能找到当前context，即如果当前fiber依赖这个workInProgress组件关联的context，并且 dependency.observedBits & changedBits , 唯一设置observedBits 的地方 readContext，observedBits是给hooks用的（useContext readContext)【TODO】 测试案例<br/>
    - 这里主要是给当前的fiber创建一个更新对象放入fiber.updateQueue中（前面这部分逻辑是针对类组件的），然后更新该fiber.expirationTime ，另外通过scheduleWorkOnParentPath方法更新所有祖先元素的childExpirationTime，用来表明存在子元素需要更新，当workInProgress完成更新后，会返回workInProgress.child作为nextUnitOfWork节点<br/>
        ...
        - 如果nextUnitOfWork自身并没有更新任务，但是其孩子节点有依赖这个context的，那么nextUnitOfWork.childExpirationTime则表明有孩子需要更新，因此workLoop会一直往下循环，直到没有某个没有更新的节点<br/>
...
- 不存在的话 并 这个fiber是个Provider
    - 如果这个fiber和workInProgress是同一个context的fiber，那么则终止循环（不继续往下深度优先遍历 why【TODO】
